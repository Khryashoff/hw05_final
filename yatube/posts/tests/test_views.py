...
class PostPagesTests(TestCase):
    ...
    def test_index_show_correct_context(self):
        ...
        expected = list(Post.objects.all()[:settings.POSTS_VOL])
    ...
    def test_group_list_show_correct_context(self):
        ...
        expected = list(
            Post.objects.filter(group_id=self.group.id)[:settings.POSTS_VOL]
        )
        self.assertEqual(list(response.context['page_obj']), expected)
    ...
    def test_profile_show_correct_context(self):
        ...
        expected = list(
            Post.objects.filter(author_id=self.user.id)[:settings.POSTS_VOL]
        )
        self.assertEqual(list(response.context['page_obj']), expected)
    ...
    def test_check_group_not_in_mistake_group_list_page(self):
        """Проверяем, что созданный Пост с группой не попап в чужую группу."""
        form_fields = {
            reverse(
                'posts:group_list', kwargs={'slug': self.group.slug}
    ...
    def test_comment_correct_context(self):
        """Проверяем валидность функционала формы комментария."""
    ...
    def test_follow_page(self):
        """
        Проверяем, что новые записи появляются у подписанного пользователя и
